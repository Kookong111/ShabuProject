...
public class OrderCustomerController {
    ...
    public ModelAndView checkAccessAndRedirectToMenu(
        ...
        if (qrToken != null && !qrToken.isEmpty()) {
            // อนุญาตให้ผ่านไป viewmenu ทันที เพื่อให้ viewmenu ไปจัดการบริบทของ Order/Table
            // ต่อ
        ...
        // 2. ถ้าไม่มีบริบท (No QR / No Session Context):
        // ต้องบังคับล็อกอินเพื่อเข้าถึงการจอง
        if (user == null) {
        ...
        if (sessionTableId == null || sessionOrderId == null) {
            ...
            mav.addObject("errorMessage",
                    "🚫 คุณยังไม่มีบิลสั่งอาหารที่เปิดใช้งานอยู่ กรุณาจองโต๊ะหรือสแกน QR Code ที่โต๊ะคุณนั่ง");
            return mav;
        }
    ...
    public ModelAndView viewmenu(HttpSession session,
        ...
        if (isScanToOrder) {
            ...
            if (activeTable != null
                    && ("Occupied".equals(activeTable.getStatus()) || "In Use".equals(activeTable.getStatus()))) {
            ...
            if (activeTable == null) {
                ...
                errorMsg = "โต๊ะ " + activeTable.getTableid()
                        + " เปิดใช้งานแล้ว แต่ไม่พบบิลสั่งอาหาร, กรุณาติดต่อพนักงานเพื่อเปิดบิล";
            } else {
                ...
                if (user != null) {
                    ...
                    // ถือว่าถ้าสแกน QR ผ่าน จะเป็น Walk-in/Scan-to-Order แต่ถ้ามี Reservation ที่
                    // Active ผูกอยู่
                    // ต้องแน่ใจว่า Order ที่สแกนตรงกับการจองของตัวเอง
                    if (reservationForOrder != null && !reservationForOrder.getTables().getTableid()
                            .equals(activeOrder.getTable().getTableid())) {
            ...
            // B. 🎯 ตรวจสอบจาก Active Reservation (ถ้าลูกค้าล็อกอินและไม่มี Session/QR
            // Code)
            ...
            if (activeReservation != null && ("Occupied".equals(activeReservation.getTables().getStatus())
                    || "In Use".equals(activeReservation.getTables().getStatus()))) {
            ...
            if (sessionOrder == null || !sessionOrder.getStatus().equals("Open")
                    || !sessionOrder.getTable().getTableid().equals(sessionTableId)) {
            ...
            if (sessionOrderId != null && activeReservation != null) {
                // Order ที่ Active ใน Session ไม่ใช่ Order ของ Reservation ที่ Active
                // ของลูกค้าคนนี้
                if (!activeReservation.getTables().getTableid().equals(sessionTableId)) {
                    errorMsg = "🚫 Order ID: " + sessionOrderId + " ไม่ตรงกับการจอง Active ของคุณ ("
                            + activeReservation.getTables().getTableid() + ")";
                    session.removeAttribute("tableId");
                    session.removeAttribute("orderId");
                }
            }
        ...
        if (errorMsg != null) {
            ...
            // หากตรวจสอบผ่านด้วย QR Code หรือ Reservation และมีการอัปเดตข้อมูล: บันทึก
            // Context ใหม่
        ...
        if (sessionTableId == null || sessionOrderId == null) {
            ...
            ModelAndView mav = new ModelAndView("orderErrorPage");
            mav.addObject("errorMessage", "⚠️ ไม่พบบิลที่เปิดใช้งานสำหรับโต๊ะนี้ กรุณาติดต่อพนักงานเสิร์ฟ");
            return mav;
        }
    ...
    public ModelAndView confirmOrder(HttpSession session) {
        ...
        try {
            ...
            if (openOrder == null || !openOrder.getStatus().equals("Open")) {
                tx.rollback();
                ModelAndView errorMav = viewCart(session);
                errorMav.addObject("error",
                        "ไม่พบบิล (Order) ที่เปิดไว้สำหรับโต๊ะที่สแกน - **กรุณาติดต่อพนักงานเสิร์ฟเพื่อเปิดบิลก่อน**");
                return errorMav;
            }
            ...
            session.setAttribute("orderSuccess",
                    "สั่งอาหารเรียบร้อยแล้ว! รายการถูกเพิ่มในบิล Order ID: " + openOrder.getOderId());
            return new ModelAndView("redirect:/viewmenu");
